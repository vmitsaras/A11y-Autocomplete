...
function normalizedSelector(value, fallback) {
...
function idIndex(root, elements) {
...
function idReferences(elements) {
	const references = [];
	for (const element of elements) for (const attribute of ID_REFERENCE_ATTRIBUTES) {
		if (!element.hasAttribute(attribute)) continue;
		const ids = new Set((element.getAttribute(attribute) ?? "").trim().split(/\s+/).filter(Boolean));
		for (const id of ids) references.push({
			attribute,
			element,
			id
		});
...
function hasValidLabelledBy(element, ids) {
	...
	return labels.every((matches) => matches.length === 1) && normalizedText(labels.map(([match]) => match?.textContent ?? "").join(" ")).length > 0;
}
function hasExplicitInputName(input, ids) {
...
function hasExplicitButtonName(button, ids) {
	return normalizedText(button.getAttribute("aria-label")).length > 0 || hasValidLabelledBy(button, ids) || normalizedText(button.textContent).length > 0;
}
...
function isInsideHiddenOrInert(element, option) {
	let current = element;
...
function isPotentialFocusTarget(element, option) {
...
function nearestStatusContainer(status, root, list, popup) {
...
function initializedRootsNear(root) {
	const selector = `[${ATTRIBUTES.initialized}]`;
	const candidates = new Set(root.ownerDocument.querySelectorAll(selector));
	if (root.matches(selector)) candidates.add(root);
	for (const element of root.querySelectorAll(selector)) candidates.add(element);
	return Array.from(candidates).filter((element) => element === root || root.contains(element) || element.contains(root));
}
function diagnoseAutocomplete(root, options = {}) {
	if (!root || root.nodeType !== 1 || root.namespaceURI !== "http://www.w3.org/1999/xhtml") throw new TypeError("diagnoseAutocomplete requires an HTMLElement root.");
	...
	if (isHtmlInput(input) && (input.type === "text" || input.type === "search")) {
		if (!hasExplicitInputName(input, ids)) issues.push({
	...
	const relevantIds = /* @__PURE__ */ new Set([...elements.map((element) => element.id).filter(Boolean), ...references.map(({ id }) => id)]);
	for (const [id, matches] of ids) if (relevantIds.has(id) && matches.length > 1) issues.push({
		code: "duplicate-id",
		message: `The id "${id}" is used by ${matches.length} elements and cannot be referenced unambiguously.`,
		elements: matches
	});
	for (const reference of references) if (!ids.has(reference.id)) issues.push({
	...
	if (clear && !isHtmlButton(clear)) issues.push({
		...
		if (!hasExplicitButtonName(clear, ids)) issues.push({
	...
	const statusCandidates = uniqueElements([...root.querySelectorAll(normalized.statusSelector), ...root.querySelectorAll("[role='status'], [role='alert'], [aria-live]:not([aria-live='off'])")]);
	...
	if (normalized.inspectRenderedOptions) {
		...
		for (const option of renderedOptions) {
			...
			if (focusTargets.length) issues.push({
	...
	for (let index = 0; index < initializedRoots.length; index += 1) for (let comparison = index + 1; comparison < initializedRoots.length; comparison += 1) {
	...
	if (normalized.log && issues.length) console.warn(`[a11y-autocomplete] Integration diagnostics found ${issues.length} issue${issues.length === 1 ? "" : "s"}.`, issues);
	return issues;
}
//#endregion
export { diagnoseAutocomplete };
